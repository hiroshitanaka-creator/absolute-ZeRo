...
test('25 service worker installation, scope-limited cleanup and explicit update lifecycle', async () => {
  ...
  const ctx = { URL, Request, console, self: { registration: { scope }, location: { origin: 'https://example.test' }, addEventListener: (type, handler) => { listeners[type] = handler; }, clients: { claim: () => { claimed++; return Promise.resolve(); } }, skipWaiting: () => skipped++ }, caches: { open: async () => ({ addAll: async files => cached.push(...files) }), keys: async () => [`absolute-zero:${scope}:0.9.0`, `absolute-zero:${scope}:1.0.0`, `absolute-zero:${scope}:1.0.1`, `absolute-zero:${scope}:1.0.2`, `absolute-zero:${scope}:1.0.3`, `absolute-zero:${scope}:1.0.4`, `absolute-zero:${scope}:1.0.5`, 'other-game:1.0', 'absolute-zero:https://example.test/elsewhere/:0.9'], delete: async k => { deleted.push(k); return true; } } };
  ...
  assert.ok(cached.every(request => request.url.startsWith(scope) && request.cache === 'reload')); assert.ok(cached.some(request => request.url === scope + 'core.js')); assert.equal(skipped, 0);
  listeners.activate({ waitUntil: p => { promise = p; } }); await promise;
  assert.deepEqual(deleted, [`absolute-zero:${scope}:0.9.0`, `absolute-zero:${scope}:1.0.0`, `absolute-zero:${scope}:1.0.1`, `absolute-zero:${scope}:1.0.2`, `absolute-zero:${scope}:1.0.3`, `absolute-zero:${scope}:1.0.4`]); assert.equal(claimed, 1);
