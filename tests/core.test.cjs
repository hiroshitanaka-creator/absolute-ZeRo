...
test('25 service worker installation, scope-limited cleanup and explicit update lifecycle', async () => {
  ...
  assert.deepEqual(deleted, [`absolute-zero:${scope}:0.9.0`, `absolute-zero:${scope}:1.0.0`, `absolute-zero:${scope}:1.0.1`, `absolute-zero:${scope}:1.0.2`, `absolute-zero:${scope}:1.0.3`]); assert.equal(claimed, 1);
