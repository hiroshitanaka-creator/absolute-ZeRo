/* Absolute Zero v1.0.0 — MIT. UI, input and local-only persistence. */
(() => {
  'use strict';
  ...
  function applySettings() { document.body.classList.toggle('dark', store.settings.dark); document.body.classList.toggle('no-motion', !store.settings.motion); document.querySelector('meta[name="theme-color"]').content = store.settings.dark ? '#14211d' : '#183c32'; }
  ...
  window.AbsoluteZero = Object.freeze({ version: '1.0.0', snapshot: () => JSON.parse(JSON.stringify({ code: state.puzzle.code, size: state.puzzle.size, board: state.board, history: state.history, redo: state.redo, stats: state.stats, status: A.status(state.board, state.puzzle.size), searching, hintMove, awarded: state.awarded })) });
})();
