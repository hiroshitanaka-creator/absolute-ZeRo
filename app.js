/* Absolute Zero v1.0.5 — MIT. UI, input and local-only persistence. */
(() => {
  'use strict';
  ...
  function render() {
    ...
    cells.forEach((cell, i) => {
      ...
      cell.classList.toggle('tile', value > 0); cell.classList.toggle('empty', !value);
      ...
      if (cell.children[0].textContent !== valueText) cell.children[0].textContent = valueText;
      const markText = !value ? '' : value >= n ? '受け' : dest.length ? dest.map(to => Math.floor(to / n) < Math.floor(i / n) ? '↑' : Math.floor(to / n) > Math.floor(i / n) ? '↓' : to > i ? '→' : '←').join('') : '·';
      if (cell.children[1].textContent !== markText) cell.children[1].textContent = markText;
  ...
  window.AbsoluteZero = Object.freeze({ version: '1.0.5', snapshot: () => JSON.parse(JSON.stringify({ code: state.puzzle.code, size: state.puzzle.size, board: state.board, history: state.history, redo: state.redo, stats: state.stats, status: A.status(state.board, state.puzzle.size), searching, hintMove, awarded: state.awarded })) });
})();
